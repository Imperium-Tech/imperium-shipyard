...
from shipyard.models.misc import Misc
...
def test_escape_pods():
    ...
    assert ship.cargo == 96
    assert ship.cost_total == 2.5

    escape_pod = Misc("Escape Pods")
    ship.add_misc(escape_pod)
    assert ship.cargo == 95.5
    assert ship.cost_total == 2.6
